...
@inline function sph_j_moment_weniger_0_1_2(config::MomentTableConfig{T, TX, V}, x, cache) where {T,TX,V}
    ν = config.ν
    return SVector{3,TX}(
...
@inline function sph_j_moment_asymptotic_0_1_2(config::MomentTableConfig{T, TX, 3}, x,
                                                    pref0, pref1, pref2) where {T,TX}
    return SVector{3,TX}(
        sph_j_moment_asymptotic_nu_3((x), 0, pref0),
        sph_j_moment_asymptotic_nu_3((x), 1, pref1),
...
@inline function sph_j_moment_asymptotic_0_1_2(config::MomentTableConfig{T, TX, V}, x,
    ...
    ν = config.ν
    return SVector{3,TX}(
        sph_j_moment_asymptotic((x), ν, 0, pref0),
        sph_j_moment_asymptotic((x), ν, 1, pref1),
        sph_j_moment_asymptotic((x), ν, 2, pref2))
end
...
function (m::MomentTable{T,TX})(x::TX) where {T,TX}
    ...
    elseif x > m.kη_max
        return sph_j_moment_asymptotic_0_1_2(
            m.config, x, m.pref0, m.pref1, m.pref2)::SVector{3,TX}
    else 
        # return sph_j_moment_weniger_0_1_2(m.config, x, m.cache)::SVector{3,TX}
...
function sph_bessel_interpolator(::Type{T}, ::Type{TX}, ν::Int, kη_min, kη_max, N::Int; 
    ...
    config = MomentTableConfig{T, TX, ν}(ν)
    ...
    for i in eachindex(xs)
        x = xs[i]
        if x < weniger_cut
            moments[i] = sph_j_moment_weniger_0_1_2(config, x, cache)
        else
            moments[i] = sph_j_moment_asymptotic_0_1_2(config, x, pref0, pref1, pref2)
        end
    end
